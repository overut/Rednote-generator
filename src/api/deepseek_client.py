...
class DeepseekAPIClient(BaseAPIClient):
    ...
    async def generate_topics(self, category: str, count: int = 5) -> List[Dict[str, str]]:
        ...
        Returns:
            选题列表
        """
        prompt = f"请为小红书平台生成{count}个关于{category}的热门选题，每个选题包括标题和简短描述，格式如下：\n1. 标题：xxx\n描述：xxx\n2. 标题：xxx\n描述：xxx\n..."
        logger.info(f"生成选题提示词: {prompt}")
        ...
        try:
            ...
            for line in lines:
                line = line.strip()
                # 处理带序号的标题行，如 "1. 标题：xxx"
                if '标题：' in line and not line.startswith('描述：'):
                    if current_topic:  # 如果已有主题，先添加到列表
                        topics.append(current_topic)
                    # 提取标题内容，去掉序号前缀
                    title_start = line.find('标题：') + 3
                    title = line[title_start:].strip()
                    current_topic = {"title": title, "description": ""}
                elif line.startswith('描述：') and current_topic:
                    current_topic["description"] = line[3:].strip()
            ...
            # 确保返回指定数量的主题
            logger.info(f"解析出的选题数量: {len(topics)}")
            return topics[:count]
            
        except Exception as e:
            logger.error(f"生成选题失败: {e}")
            # 返回一个默认选题，避免完全失败
            return [{"title": f"关于{category}的分享", "description": f"分享一些关于{category}的心得和体验"}]
